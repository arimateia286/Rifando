...
checkboxParticipante.addEventListener('change', () => {
	if (checkboxParticipante.checked) {
		participantes = Object.entries(rifaPoints)
			.filter(([numero, dados]) => dados.status === 'Pago' || dados.status === 'Não pago')
			.map(([numero, dados]) => ({ numero, dados }));
		updateInfosRifa();
	} else {
		participantes = Object.entries(rifaPoints)
...
const vendidos = Object.values(rifaPoints)
	.filter(ponto => ponto.status == 'Pago' || ponto.status == 'Não pago');
...
if (participantes.length > 1) {
	...
	const participantesTable = document.querySelector('#participantes-table tbody');
	...
	`;
		...
		} else tr.innerHTML += `<td class="status-nao-pago">${ponto.dados.status}</td>`;
	...
	verButton.addEventListener('click', () => {
		participantesDialog.style.display = 'flex';
		participantesDialog.showModal();
	});

	function closeParticipantesDialog() {
		participantesDialog.close();
		participantesDialog.style.display = 'none';
	}
} else {
	alert('É preciso pelo menos 2 participantes para fazer o sorteio!')
		.then(result => {
			goHome();
		});
}
